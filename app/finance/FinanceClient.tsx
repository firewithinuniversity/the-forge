                    ...
                    return (
                      <tr key={t.id} className="border-b border-[#27272A]/50 hover:bg-[#1A1A1E]/50 transition-[background-color] animate-stagger-in" style={{ animationDelay: `${index * 30}ms` }}>
                        ...
                        <td className="py-3 px-3 text-center">
                          <ReceiptButton transactionId={t.id} saved={t.receiptSaved} hasFile={t.hasReceiptFile ?? false} />
                        </td>
                        <td className="py-3 px-3 text-center">
...
/* ─── Receipt Button (upload / view / delete) ─── */
function ReceiptButton({ transactionId, saved, hasFile }: { transactionId: string; saved: boolean; hasFile: boolean }) {
  const router = useRouter();
  ...
  async function handleDelete() {
    setLoading(true);
    try {
      await fetch(`/api/transactions/${transactionId}/receipt`, { method: "DELETE" });
      router.refresh();
    } catch { alert("Failed to remove receipt"); }
    finally { setLoading(false); setShowMenu(false); }
  }
  ...
  if (loading) {
    return (
      <div className="inline-flex items-center rounded-full px-2 py-0.5 text-[10px] text-[#52525B]">
        <div className="h-3 w-3 border border-[#52525B]/30 border-t-[#52525B] rounded-full animate-spin" />
      </div>
    );
  }

  // saved=true, hasFile=true  → green "Saved" badge, dropdown: View / Replace / Remove
  // saved=true, hasFile=false → amber "Marked" badge, dropdown: Upload file / Unmark
    ...
    <div className="relative">
      ...
      <button
        ...
        className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium transition-colors ${
          saved && hasFile
            ? "bg-green-500/15 text-green-400 [@media(hover:hover)_and_(pointer:fine)]:hover:bg-green-500/25"
            : saved && !hasFile
            ? "bg-amber-500/15 text-amber-400 [@media(hover:hover)_and_(pointer:fine)]:hover:bg-amber-500/25"
            : "bg-[#27272A] text-[#52525B] [@media(hover:hover)_and_(pointer:fine)]:hover:text-[#A1A1AA] [@media(hover:hover)_and_(pointer:fine)]:hover:bg-[#3F3F46]/50"
        }`}
        title={saved && hasFile ? "View receipt" : saved ? "Marked as saved — no file uploaded" : "Upload receipt"}
      >
        {saved && hasFile ? (
          <>
        ...
        >
          {saved && hasFile ? (
            <>
              <button
                onClick={handleView}
                className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-[#FAFAFA] hover:bg-[#27272A] transition-colors"
              >
              ...
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-red-400 hover:bg-[#27272A] transition-colors"
              >
                <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            <>
              ...
              <button
                onClick={handleDelete}
                className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-amber-400 hover:bg-[#27272A] transition-colors"
              >
                <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
